...
var csPlayer = {
  ...
  preSetup: (videoTag, playerTagId, defaultId) => {
    var theme =
      "theme" in csPlayer.csPlayers[videoTag]["params"]
        ? csPlayer.csPlayers[videoTag]["params"]["theme"]
        : null;
 ...
 <div class="csPlayer-settings-box">
  ...
  YtSetup: (videoTag, playerTagId, defaultId) => {
        ...
        events: {
          onReady: () => {
            if ($("#" + videoTag) != null && videoTag) {
              csPlayer
                .pauseVideoWithPromise(csPlayer.csPlayers[videoTag]["videoTag"])
                .then(() => {
                  parent
                    .querySelector(".csPlayer-container iframe")
                    .addEventListener("load", () => {
                      parent
                        .querySelector(".csPlayer-container span i")
                        .classList.remove("csPlayer-loading");
                      csPlayer.csPlayers[videoTag]["videoTag"].addEventListener(
                        "onStateChange",
                        onPlayerStateChange,
                      );
                      parent
                        .querySelector(
                          ".csPlayer-controls-box main i:nth-of-type(1)",
                        )
                        .addEventListener("click", backward);
                      parent
                        .querySelector(
                          ".csPlayer-controls-box main i:nth-of-type(2)",
                        )
                        .addEventListener("click", togglePlayPause);
                      parent
                        .querySelector(
                          ".csPlayer-controls-box main i:nth-of-type(3)",
                        )
                        .addEventListener("click", forward);
                      csPlayer.csPlayers[videoTag]["TextTimeInterval"] =
                        setInterval(updateTextTime, 1000);
                      csPlayer.csPlayers[videoTag]["TimeSliderInterval"] =
                        setInterval(updateTimeSlider, 1000);
                      parent
                        .querySelector(
                          ".csPlayer-controls-box .csPlayer-controls input",
                        )
                        .addEventListener("input", updateSlider);
                      parent
                        .querySelector(
                          ".csPlayer-controls-box .csPlayer-controls .fsBtn",
                        )
                      ...
                      parent
                        .querySelector(
                          ".csPlayer-controls-box .csPlayer-controls .settingsBtn",
                        )
    ...
    function backward() {
      ...
      controlsTimeout = setTimeout(() => {
        parent
          .querySelector(".csPlayer-controls-box")
          .classList.remove("csPlayer-controls-open");
      }, 3000);
    }
    //forward
    function forward() {
    ...
    function updateTextTime() {
      ...
      parent.querySelector(
        ".csPlayer-controls-box .csPlayer-controls p:nth-of-type(2)",
      ).innerHTML = formatTime(String(duration));
    }
    //update slider
    ...
    function resetSettings() {
      var settings = parent.querySelector(
        ".csPlayer-controls-box .csPlayer-settings-box",
      );
    ...
    function toggleSettings() {
      ...
      var qualities =
        csPlayer.csPlayers[videoTag]["videoTag"].getAvailableQualityLevels();
      for (x of qualities) {
        if (
          !settings.querySelector("span:nth-of-type(2)").innerHTML.includes(x)
        ) {
          settings.querySelector("span:nth-of-type(2)").innerHTML +=
            `<label><input type="radio" name=${videoTag}2>${x}</label>`;
        }
      }
      ...
      settings
        .querySelectorAll("span:nth-of-type(1) input")
        .forEach((spdInput) => {
          spdInput.addEventListener("change", (e) => {
            var value = e.target.parentElement.innerText.slice(0, -1);
            settings.querySelector("p:nth-of-type(1) b").innerText =
              value + "x";
            csPlayer.csPlayers[videoTag]["videoTag"].setPlaybackRate(
              Number(value),
            );
          });
        });
      ...
      settings
        .querySelectorAll("span:nth-of-type(2) input")
        .forEach((qualInput) => {
          qualInput.addEventListener("change", (e) => {
            ...
            try {
              ...
              csPlayer.csPlayers[videoTag]["videoTag"].setPlaybackQuality(
                value,
              );
    ...
    function onPlayerStateChange(event) {
      if (event.data == YT.PlayerState.PLAYING) {
        ...
        parent.querySelector(
          ".csPlayer-controls-box main .csPlayer-play-pause-btn",
        ).className = "ti csPlayer-play-pause-btn ti-player-pause-filled";
        parent
          .querySelector(".csPlayer-container span i")
          .classList.add("csPlayer-loading");
        parent.querySelector(".csPlayer-container span").style.display = "none";
        csPlayer.csPlayers[videoTag]["videoTag"].unMute();
        parent.querySelector(".csPlayer-container").style.pointerEvents =
          "none";
        ...
        parent.querySelector(".csPlayer-controls-box").onclick = function (e) {
          if (
            !parent
              .querySelector(".csPlayer-controls-box main")
              .contains(e.target) &&
            !parent
              .querySelector(".csPlayer-controls-box .csPlayer-controls")
              .contains(e.target) &&
            !parent
              .querySelector(".csPlayer-controls-box .csPlayer-settings-box")
              .contains(e.target)
          ) {
            if (
              parent
                .querySelector(".csPlayer-controls-box")
                .classList.contains("csPlayer-controls-open")
            ) {
              parent
                .querySelector(".csPlayer-controls-box")
                .classList.remove("csPlayer-controls-open");
              ...
              parent
                .querySelector(".csPlayer-controls-box")
                .classList.add("csPlayer-controls-open");
              clearTimeout(controlsTimeout);
              controlsTimeout = setTimeout(() => {
                parent
                  .querySelector(".csPlayer-controls-box")
        ...
        parent.querySelector(
          ".csPlayer-controls-box main .csPlayer-play-pause-btn",
        ).className = "ti csPlayer-play-pause-btn ti-player-play-filled";
        if (
          !parent
            .querySelector(".csPlayer-controls-box")
            .classList.contains("csPlayer-controls-open")
        ...
        if (
          csPlayer.csPlayers[videoTag]["params"]["loop"] == true ||
          csPlayer.csPlayers[videoTag]["params"]["loop"] == "true"
        ) {
        ...
        if ($("#" + videoTag) != null) {
          if (!(videoTag in csPlayer.csPlayers)) {
            ...
            if ("defaultId" in params) {
              csPlayer.csPlayers[videoTag]["params"]["defaultId"] =
                params["defaultId"];
            }
            if ("loop" in params) {
            ...
            if ("thumbnail" in params) {
              csPlayer.csPlayers[videoTag]["params"]["thumbnail"] =
                params["thumbnail"];
            }
            if ("theme" in params) {
            ...
            csPlayer
              .preSetup(
                videoTag,
                (playerTagId = "csPlayer-" + videoTag),
                params["defaultId"],
              )
              .then(() => {
                var parent = document
                  .querySelector("#" + playerTagId)
                  .closest(".csPlayer");
                if ("thumbnail" in csPlayer.csPlayers[videoTag]["params"]) {
                  if (
                    ...
                    parent.querySelector(
                      ".csPlayer-container span",
                    ).style.backgroundImage =
                      `url("https://img.youtube.com/vi/${csPlayer.csPlayers[videoTag]["params"]["defaultId"]}/maxresdefault.jpg")`;
                  } else if (
                    csPlayer.csPlayers[videoTag]["params"]["thumbnail"] ==
                      false ||
                    csPlayer.csPlayers[videoTag]["params"]["thumbnail"] ==
                    ...
                    parent.querySelector(
                      ...
                      `url(${csPlayer.csPlayers[videoTag]["params"]["thumbnail"]})`;
                  }
                }
                csPlayer
                  .YtSetup(
                  ...
                  .then(() => {
                    ...
                    if (csPlayer.csPlayers[videoTag]["params"]["autoPlay"] == true) {
                        setTimeout(() => {
                            csPlayer.csPlayers[videoTag]["videoTag"].playVideo();
                        }, 1000);
                    }
          ...
          throw new Error(
            "No tag with id " + videoTag + " available in the document.",
          );
        }
      } else {
        throw new Error(
          "Init function must have two parameters and second parameter must have defaultId.",
        );
      }
      resolve();
    });
    ...
    if (videoTag) {
      if (
        ...
        if (!csPlayer.csPlayers[videoTag]["videoTag"].isMuted()) {
          ...
          throw new Error(
            "Before calling play function, the video must be played atleat once.",
          );
        }
      } else {
        throw new Error("Player " + videoTag + " is not initialized yet.");
  ...
  getDuration: (videoTag) => {
    if (videoTag) {
      ...
      throw new Error(
        "getDuration function must have player id as a parameter.",
      );
    }
  },
  getCurrentTime: (videoTag) => {
    if (videoTag) {
      ...
      throw new Error(
        "getCurrentTime function must have player id as a parameter.",
      );
    }
  },
  getVideoTitle: (videoTag) => {
    if (videoTag) {
      ...
      throw new Error(
        "getVideoTitle function must have player id as a parameter.",
      );
    }
  },
  getPlayerState: (videoTag) => {
    if (videoTag) {
      ...
      throw new Error(
        "getPlayerState function must have player id as a parameter.",
      );
    }
  },
  changeVideo: (videoTag, videoId, start = 0) => {
    if (videoTag && videoId) {
      ...
      throw new Error(
        "changeVideo function must have two parameters, first parameter as player Id and second as the new YouTube video ID.",
      );
    }
  },
  destroy: (videoTag) => {
